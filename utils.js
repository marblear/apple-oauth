...
export async function getServiceConfiguration({ appId = null }) {
  ...
  const service = ServiceConfiguration.configurations.findOneAsync
    ? await ServiceConfiguration.configurations.findOneAsync({ service: 'apple' })
    : ServiceConfiguration.configurations.findOne({ service: 'apple' });
  if (!appId) {
    return service;
  }
  if (
